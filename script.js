    ...
    movies.forEach((movie) => {
        ...
        movieEl.innerHTML = `
            ...
            <div class="overview">
                <h3>Overview</h3>
                ${overview}
                <a href="${getDownloadLink(title)}" target="_blank" class="download-link">Download</a>
            </div>
        `;
        main.appendChild(movieEl);
    });
...
function getDownloadLink(title) {
    ...
    const downloadBaseUrl = 'https://m.vegamovies.tips/';
    const encodedTitle = encodeURIComponent(title);
    return downloadBaseUrl + encodedTitle;
}
...
const joinUsButton = document.querySelector('nav ul li:nth-child(4) a');
